...
and crypto (Ethereum).  Interest in security and programming languages.
EOF
...
_2024 -> Present_, Caldera is an Ethereum infrastructure provider offering rollups-as-a-service for
customer-operated chains.

**Scope**: Greenfield platform engineering for multi-tenant blockchain workloads. Kubernetes, IaC, deployment
automation, monitoring, security, and product expansion.

**Impact**
- **IaC** for supported chain stacks and supporting services with deep per-stack customization
- Multi-tenant **Kubernetes** hosting hundreds of customers with strong security and cost discipline
- **Workflow orchestration** for reliable deployment and operation across customer environments
- Evangelized **security** within engineering. Drove customer infra hardening, key management improvements, and engineering operations security
...
_2022 -> 2024_, Panther is a cloud-native SIEM for threat detection with hundreds of native integrations and
customization via Python or no-code rules.

**Scope**: Deployment automation and internal tooling for a roughly 80-person engineering org.
...
_2021 -> 2022_, ClearLabs: gene sequencing for public and private sector; whole genome, clinical virology,
...
- SD-WAN **networking** implementation to support deployment behind diverse customer networks
...
_2019 -> 2021_, Imagine Financial: payment processing startup focused on merchant-friendly, lower-fee
routing.

**Scope**: First engineering hire; owned backend and cloud from zero through production launch.

**Impact**
- Owned **product backend** (Go APIs, Postgres) and **AWS** infrastructure end to end
...
_2016 -> 2019_, Jobs and Companies, then Recruiter; reliability, observability, and on-call at LinkedIn.

**Scope**: High-throughput web products: infrastructure design, monitoring, and incident response.
...
_2012 -> 2016_, Student Linux admin at Michigan Tech: standard campus images, software packaging, and
automated rollout to labs, faculty/staff desktops, and research systems.

**Scope**: Linux platform for university labs, offices, and research environments.
